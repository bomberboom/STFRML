function result = SVM_classifier(data_feature,test_type,n_fold,numTrain,numRound)

if test_type %n-fold cross validation
    result = cross_valid(data_feature,n_fold,numRound);
else
    result = split_valid(data_feature,numTrain,numRound);
end
end

%------------------cross validation function------------%
function result = cross_valid(data_feature,n_fold,numRound)

len_feature_21=size(data_feature_Testing(1).feature_21,2);
samp_num = length(data_feature);
feature_4 = zeros(samp_num,4);
feature_21 = zeros(samp_num,len_feature_21);
...
function result = split_valid(data_feature,numTrain,numRound)

oldDim=length(data_feature(1).SPD);
newDim=22;%16时识别率最高
numClass=6;
epoch=50;
kw=15;
kb=10;
%load("D:\research\Undergraduate Graduation Project\bishe\workSpace\OATH_inception_resnetv2_mixed_5b.mat")
%SPD_python=SPD;

ex = zeros(numRound,1);
indices = crossvalind('Kfold',length(data_feature),10);
for i = 1:numRound
    test_ind = (indices == i);%测试集编号
    train_ind = ~test_ind;
    
    %% 降维
    [covD_Struct,R]=addfeature_reconstruct_OATH(data_feature,oldDim,newDim,numClass,numTrain,numRound,i,test_ind,train_ind);
    [TL_trnX,TL_tstX,U]=my_runme(covD_Struct,newDim,epoch,kw,kb);
    log_TL_trnX=zeros(newDim,newDim,length(covD_Struct.trn_X));
    log_TL_tstX=zeros(newDim,newDim,length(covD_Struct.tst_X));
...
% % %     不降维
%     covD_Struct=addfeature_reconstruct_OATH(data_feature,oldDim,newDim,numClass,numTrain,numRound,i,test_ind,train_ind);
%     log_TL_trnX=zeros(oldDim,oldDim,length(covD_Struct.trn_X));
%     log_TL_tstX=zeros(oldDim,oldDim,length(covD_Struct.tst_X));
% 
%     for countvariable=1:1:length(covD_Struct.trn_X)
%         log_TL_trnX(:,:,countvariable)=logpsd(covD_Struct.trn_X(:,:,countvariable));
%     end
%     for countvariable=1:1:length(covD_Struct.tst_X)
%         log_TL_tstX(:,:,countvariable)=logpsd(covD_Struct.tst_X(:,:,countvariable));
%     end
%    [train_data,test_data]=spd2vector(log_TL_trnX,log_TL_tstX);
   %%%[train_data,test_data]=spd2vector(covD_Struct.trn_X,covD_Struct.tst_X);%不log

    %%  split training set and testing set
    train_label = covD_Struct.trn_y';
    test_label_true = covD_Struct.tst_y';
    class_num = length(unique([test_label_true;train_label])); %总的云图类别数量
    classNum=class_num;
    confusion_rat = zeros(class_num,class_num,numRound);%？
    ...
    %% SVM
    [model class_num]= SupportVectorTrain(train_data,train_label,4,classNum);
    test_label = SupportVector(test_data,model,class_num);
    [~ , max_pos] = max(test_label');
    test_label_SVM = max_pos';
    %% show results
    clear train
    for j = 1:size(train_data,1)
        %train(j).name = data_feature(train_ind(j)).name;
    ...
    for j =1:size(test_data,1)
        %test(j).name = data_feature(test_ind(j+size(train_data,1))).name;
...
result(1).ex_mean = mean(ex);%%overall accuracy by numRound
result(1).confusion_num_mean = mean(confusion_num,3);%重复多次的分类，不能用以下两行，需额外处理
