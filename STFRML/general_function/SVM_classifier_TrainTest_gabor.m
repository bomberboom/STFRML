...
%feature_12 = zeros(samp_num,12);
%len_feature_21=size(data_feature_Testing(1).feature_21,2);
%feature_21 = zeros(samp_num,len_feature_21);%manifold-feature，21维
kind = zeros(samp_num,1);%各云图的类型，通过编号表示
% gabor_feature_Test=load('gaborpure_Test_gray_8_6_dspl_vectorized.mat');
% gabor_feature_Test=gabor_feature_Test.gabor_origin;
% 
% gabor_feature_Train=load('gaborpure_Train_gray_8_6_dspl_vectorized.mat');
% gabor_feature_Train=gabor_feature_Train.gabor_origin;

%gabor_feature=load('.\TrainingImages_gabor_LG1_512.mat');
%gabor_feature=gabor_feature.gabor_feature_LG1;
%data_feature=data_feature_Testing;
for i = 1:samp_num
    %feature_4(i,:) = data_feature(i).feature_GLCM;
    %feature_12(i,:) = data_feature(i).feature_GLCM;
    %feature_21(i,:) = data_feature(i).feature_21;
    %gabor(i,:)=map2IDS_vectorize(gabor_feature_train(:,:,i),0);
    %gabor(i,:)=data_feature(i).gabor;
    kind(i,:) = data_feature(i).kind+1;%避免第0类无法索引
    %gabor_feature(i,:)=data_feature(i).gabor;
end

%feature = [feature_21,feature_4];%融合texture和manifold feature
...
for i = 1:1:1
   % [train_feature,train_kind,test_feature,test_kind,train_index(:,i),test_index(:,i)] = sample_extract_new(feature,kind,numTrain);
    ...
    %feature_21 = zeros(samp_num,len_feature_21);%manifold-feature，21维
    kind = zeros(samp_num,1);%各云图的类型，通过编号表示
    %gabor=zeros(samp_num,105);
    %feature_12 = zeros(samp_num,12);
    %gabor_feature=load('.\TestingImages_gabor_LG1_512.mat');
    %gabor_feature=gabor_feature.gabor_feature_LG1;
    for i = 1:samp_num
        %feature_4(i,:) = data_feature(i).feature_GLCM;
        %feature_12(i,:) = data_feature(i).feature_GLCM;
        %feature_21(i,:) = data_feature(i).feature_21;
        kind(i,:) = data_feature(i).kind+1;%避免第0类无法索引
        %gabor(i,:)=map2IDS_vectorize(gabor_feature_Test(:,:,i),0);
        %gabor(i,:)=data_feature(i).gabor;
        %gabor_feature(i,:)=data_feature(i).gabor;
    end
    i=1;
    %feature = [feature_21,feature_4];
    %test_data = feature;
    test_label_true = kind;
    
    for leaf=1:1:length(data_feature_Training)
        trn_X(:,:,leaf)=data_feature_Training(leaf).SPD;
    end
    for leaf=1:1:length(data_feature_Testing)
        tst_X(:,:,leaf)=data_feature_Testing(leaf).SPD;
    end
    %% 降维
    covD_Struct.trn_X=trn_X;
    covD_Struct.trn_y=train_label;
    covD_Struct.tst_X=tst_X;
    covD_Struct.tst_y=test_label_true;
    covD_Struct.nClasses=7;
    covD_Struct.n=length(data_feature(1).SPD);
    covD_Struct.r=7;
    [TL_trnX,TL_tstX,U]=my_runme(covD_Struct,newDim,50,kw,kb);
